...
import { uploadSource, useGetKbItems } from 'src/api/kmm';

import { fileFormat } from 'src/components/file-thumbnail';
import { SkeletonChartTable } from 'src/components/skeleton';
import { useTable, getComparator } from 'src/components/table';

import { IDataset, KbItemManager, IKbTableFilters, IDatasetFilterValue } from 'src/types/dataset';
...
export default function KbManagerRoot({ kbMeta, refreshKey, openUpload, onCloseUpload }: Props) {
  ...
  const { kbItems, isLoading } = useGetKbItems(kbMeta.id, refreshKey);
  ...
  const handleUpload = async (files: File[]) => {
    files.forEach((file, _) => {
      ...
      const additionalData = { fileName: file.name, id: fileId, size: file.size };
      const _row = {
        id: fileId,
        name: file.name,
...
function applyFilter({
  ...
  if (statuses.length) {
    inputData = inputData.filter((file) => statuses.includes(fileFormat(file.status)));
  }
