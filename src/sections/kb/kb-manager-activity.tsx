...
export default function KbManagerActivity({ title, subheader, chart, ...other }: Props) {
  ...
  const chartOptions = useChart({
    ...
    plotOptions: {
      bar: {
        borderRadius: (seriesData === 'Week' && 3) || (seriesData === 'Month' && 2) || 4,
        columnWidth: '20%',
      },
    },
    ...options,
  ...
  return (
    <>
      <Card {...other}>
        ...
        {series.map((item) => (
          <Box key={item.type} sx={{ mt: 3, ml: 1, mr: 2, mb: -2 }}>
