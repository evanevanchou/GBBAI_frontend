...
import ChatMessageItemMatchHandler from './ChatMessageItemMatchHandler';
import ChatMessageItemResturantOrderHandler from './ChatMessageItemResturantOrderHandler';
import ChatMessageItemResturantOrderOkHandler from './ChatMessageItemResturantOrderOkHandler';
...
export default function ChatMessageItemFuncHandler({ function_calls }: Props) {
  ...
  if (funcName === 'get_stock_data') {
    ...
    return <ChatMessageItemMatchHandler data={JSON.parse(function_calls[0].results)} />;
  } else if (funcName === 'get_resturant_list') {
    return <ChatMessageItemResturantOrderHandler data={JSON.parse(function_calls[0].results)} />;
  } else if (funcName === 'book_resturant') {
    return <ChatMessageItemResturantOrderOkHandler data={JSON.parse(function_calls[0].results)} />;
  } else return null;
}
